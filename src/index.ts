...
module angularPoint {
   ...
   * @description
   */
  export class apLookupCacheService {
    lookupCache = {};
     ...
     */
    cacheEntityByLookupId(entity:ListItem, propertyArray:string[]) {
      var self = this;
      if (entity.id) {
    ...
    removeEntityFromLookupCaches(entity:ListItem, propertyArray:string[]) {
      var self = this;
      if (entity.id) {
        var listId = entity.getListId();
        /** Only cache entities saved to server */
    ...
    private cacheSingleLookup(entity:ListItem, propertyName:string, listId:string) {
      var self = this;
      ...
      _.each(lookups, function (lookup:Lookup) {
    ...
    private removeEntityFromSingleLookupCache(entity:ListItem, propertyName:string, listId:string) {
      var self = this;
      /** Handle single and multiple lookups by only dealing with an Lookup[] */
      var lookups = _.isArray(entity[propertyName]) ? entity[propertyName] : [entity[propertyName]];
      _.each(lookups, function (lookup:Lookup) {
    ...
    private getLookupCache(propertyName:string, listId:string) {
  ...
  angular
      .module('angularPoint')
      .service('apLookupCacheService', apLookupCacheService);
